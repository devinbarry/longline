...
fn test_bare_assign_date() {
    let result = run_claude_hook("Bash", "VAR=$(date)");
...
fn test_bare_assign_pwd() {
    let result = run_claude_hook("Bash", "DIR=$(pwd)");
    assert_eq!(result.exit_code, 0);
    result.assert_claude_decision("allow");
}
...
fn test_bare_assign_plain_value() {
    let result = run_claude_hook("Bash", "VAR=hello");
    assert_eq!(result.exit_code, 0);
    result.assert_claude_decision("allow");
}
...
fn test_bare_assign_dangerous_rm() {
    let result = run_claude_hook("Bash", "VAR=$(rm -rf /)");
    assert_eq!(result.exit_code, 0);
...
fn test_bare_assign_unknown_command() {
    let result = run_claude_hook("Bash", "VAR=$(unknown_tool)");
    assert_eq!(result.exit_code, 0);
    result.assert_claude_decision("ask");
}
