mod support;
use support::claude::{run_claude_hook, run_claude_hook_with_flags, ClaudeRunResultExt};
...
fn test_e2e_trust_level_minimal_allows_readonly() {
    let result = run_claude_hook_with_flags("Bash", "ls -la", &["--trust-level", "minimal"]);
    assert_eq!(result.exit_code, 0);
...
fn test_e2e_trust_level_full_allows_full_tier() {
    let result = run_claude_hook_with_flags("Bash", "git gc", &["--trust-level", "full"]);
    assert_eq!(result.exit_code, 0);
    result.assert_claude_decision("allow");
}
...
fn test_e2e_trust_level_standard_asks_git_push() {
    let result = run_claude_hook("Bash", "git push origin main");
    assert_eq!(result.exit_code, 0);
    result.assert_claude_decision("ask");
}
...
fn test_e2e_safety_level_flag_overrides_config() {
    ...
    let result =
        run_claude_hook_with_flags("Bash", "chmod 777 /tmp/f", &["--safety-level", "critical"]);
    assert_eq!(result.exit_code, 0);
